...
URL = 'https://www.gutenberg.org/cache/epub/pixar/pg2554.txt'
# Запасной вариант — Достоевский, Преступление и наказание
URL = 'https://www.gutenberg.org/files/2554/2554-0.txt'

response = requests.get(URL, timeout=30)
raw = response.content.decode('utf-8', errors='ignore')

# Убираем шапку Gutenberg
start = re.search(r'\*\*\* START OF', raw)
end   = re.search(r'\*\*\* END OF',   raw)
if start and end:
    full_text = raw[raw.find('\n', start.end())+1 : end.start()]
else:
    full_text = raw

# Берём первые ~70k символов
text = full_text[:70000]

print(f'Взяли фрагмент: {len(text):,} символов')
...
print()
print('Вывод: regex быстрее и находит больше вариантов,\n'
      'spaCy Matcher точнее — учитывает грамматику и часть речи.')
